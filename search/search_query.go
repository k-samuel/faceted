...
func (q *SearchQuery) Filter(f FilterInterface) *SearchQuery {
	q.filters = append(q.filters, f)
	return q
}

// Filters adds multiple filters to the query.
func (q *SearchQuery) Filters(filters []FilterInterface) *SearchQuery {
	q.filters = append(q.filters, filters...)
	return q
}

// Order sets the sorting order for results.
func (q *SearchQuery) Sort(fieldName string, direction int, sortType int) *SearchQuery {
	q.sort = NewSort(fieldName, direction, sortType)
	return q
}
...
func (q *SearchQuery) GetFilters() []FilterInterface {
	return q.filters
}
